...
export const CircleIconButton = styled(IconButton)`
    ...
    /* padding: 0.5em; */
    aspect-ratio: 1/1;
    width: clamp(2rem, 1vw + 2rem, 2.5rem);
    height: clamp(2rem, 1vw + 2rem, 2.5rem);
    display: grid;
    place-items: center;
    /* width: 2rem; */
    /* height: 2rem; */
...
export const ButtonWithIcon = ({ type, icon, ...props }) => {
    // const iconEl = <span className="material-symbols-outlined">{icon}</span>;
    const iconEl = <Icon icon={icon}></Icon>
    return (
        <>
            {type === "circle" ? (
                <CircleIconButton {...props}>{iconEl}</CircleIconButton>
            ) : (
                <IconButton {...props}>{iconEl}</IconButton>
            )}
        </>
    );
};
